...
// Represents a trophy in the game.
typedef struct {
    Point position;
    int value;
    int active;
...
// Ends the game and displays a "Congratulations! You've won the game!" message.
void winGame() {
    clear();
...
// Moves the snake by updating its body segments based on the current direction.
...
// Changes the direction of the snake based on the user input key, preventing opposite or out-of-bounds movements.
...
// Draws the trophy on the screen if it is active.
...
// Generates a new trophy at a random position within the game area.
...
// Expires the current trophy, making it inactive.
